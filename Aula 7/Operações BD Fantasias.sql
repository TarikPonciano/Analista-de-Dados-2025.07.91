...
/* -------------------------
   TABELA: Cliente
   Armazena os dados dos clientes que realizam aluguéis
   ------------------------- */
CREATE TABLE "Cliente" (
    cod_cliente SERIAL PRIMARY KEY,   -- Identificador único do cliente
    cli_nome     VARCHAR(100) NOT NULL, -- Nome completo do cliente
    cli_cpf      VARCHAR(11)  NOT NULL UNIQUE, -- CPF do cliente (único)
    cli_altura   NUMERIC(3,2),          -- Altura do cliente (ex: 1.75)
    cli_nasc     DATE                   -- Data de nascimento
);


/* -------------------------
   TABELA: Traje
   Armazena os trajes disponíveis para aluguel
   ------------------------- */
CREATE TABLE "Traje" (
    cod_traje SERIAL PRIMARY KEY,     -- Identificador único do traje
    nome      VARCHAR(100) NOT NULL,  -- Nome do traje
    tipo      VARCHAR(50),            -- Tipo (Formal, Festa, Fantasia etc.)
    tamanho   VARCHAR(5),             -- Tamanho (P, M, G, GG)
    valor     NUMERIC(10,2) NOT NULL, -- Valor do aluguel
    genero    VARCHAR(20)             -- Gênero associado ao traje
);
...
/* -------------------------
   ...
   entre Cliente e Traje
   ------------------------- */
CREATE TABLE "Aluguel" (
    cod_aluguel    SERIAL PRIMARY KEY, -- Identificador do aluguel
    cod_cliente    INTEGER NOT NULL,   -- Cliente que alugou
    cod_traje      INTEGER NOT NULL,   -- Traje alugado
    data_aluguel   DATE NOT NULL,      -- Data em que o traje foi alugado
    data_devolucao DATE,               -- Data da devolução (pode ser nula)
    situacao       VARCHAR(20)       -- Situação do aluguel

	-- Ainda não usaremos a configuração de chave estrangeira, mas segue exemplo abaixo:
    ...
    -- -- Chave estrangeira para Traje
    -- CONSTRAINT fk_traje
    --     FOREIGN KEY (cod_traje)
    --     REFERENCES "Traje"(cod_traje)
...
/* Inserção parcial (somente nome e CPF) */
INSERT INTO "Cliente" (cli_nome, cli_cpf)
VALUES 
('Maria Rosário', '12345678944'),
('Jão Silva',     '32165498778');

/* Inserção em lote de vários clientes */
